...
```bash
target/debug/devforge --project <project> install framework --framework ../DevForgeAI --provider claude --include-experts --runtime <single-link copy of devforge>
```
...
The compiled `devforge install framework` is the Rust port of `scripts/install_framework.py`'s project installation (see [framework installation](integration/framework-installation.md)); the Python script is retained unchanged as the legacy baseline and oracle. `--provider both` and `--provider codex` select the promoted Codex expert packages, which install only through `devforge install manual-experts` with owner-selected adoption evidence and an owner-controlled `--authority` record (the compiled path for the legacy `--manual-evidence`/`--manual-experts-only` mode). A delivery-aware package additionally requires `--runtime <absolute devforge>`; the executable running the command is the validating authority and applies the probe and its own pre-write protections in process. The legacy script still selects `--runtime` and `--validator`, two independently selected executables (the same file is permitted), and runs `<validator> --project <project> install probe-runtime --runtime <runtime> --provider <provider>`; that compiled path owns the runtime hygiene checks, the bounded `delivery capabilities` execution and the capability contract, admitting only the eight-field `devforge.delivery-capabilities/v1` base set or that base plus all seven declared extensions. The named project is refused before anything is read or executed unless the validating executable is outside it, and the report records that project so the installer can require the evidence it writes to be bound to the installation it is performing. Immediately before any write the installer then runs `<validator> --project <project> install guard-validator`, passing that probe report and every planned destination as one `devforge.validator-guard-request/v1` object on stdin; that compiled path decides the validator's own pre-write protections — no planned destination may name it, no existing destination may already alias its inode, and its digest must still equal the identity the report bound — and writes nothing. Python only supplies the inputs and propagates the refusal. The Python installer validates nothing itself, records the probe report as the runtime evidence, and writes nothing when the Rust CLI refuses. Note that `cargo` hard-links `target/debug/devforge`, so select a single-link copy for either role.

For a Claude native plugin test, first export to a new directory whose basename is devforgeai:

```bash
target/debug/devforge install export-plugin --framework <framework> --provider claude --output <new-export-parent>/devforgeai
