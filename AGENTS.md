...
Most CLI acceptance tests are Python `unittest` cases launching the real compiled binary; Rust unit tests cover internal behavior. Rebuild with `cargo build --locked` before CLI RED/GREEN runs, including changes to embedded Python runtime sources. Some tests hardcode `target/debug/devforge`; a `DEVFORGE_BIN` override does not cover every suite.
